...
3. Brak opcjonalnego narzędzia zewnętrznego (Audiveris, FFmpeg, Tesseract) daje źródłu status „blad”, nie „pominiete”. Objaw: zauważone przy niezależnej weryfikacji pull requesta 28. Ogólny dysponent w `gnb/potok.py`, metoda `przetworz`, rozpoznaje jako świadome pominięcie wyłącznie wyjątki `PominietoZrodlo` i `PrzekroczonoLimit`; `BrakNarzedzia` przechodzi ogólną gałęzią `except BladGnb` i dostaje status błędu. Przyczyna: `BrakNarzedzia` nigdy nie było jawnie zestawione z pozostałymi dwoma wyjątkami przy projektowaniu tego mechanizmu we wcześniejszych etapach — to nie jest decyzja podjęta świadomie, tylko luka. Skutek praktyczny: przebieg, w którym dwadzieścia skanów nie zostało rozpoznanych wyłącznie dlatego, że Audiveris nie jest zainstalowany, raportuje „dwadzieścia błędów”, co dla kogoś słuchającego podsumowania czytnikiem ekranu brzmi jak awaria, a sekcja piąta punkt siódmy każe traktować brak narzędzia opcjonalnego jako „wyłączenie konkretnej ścieżki przetwarzania”, bliższe pominięciu niż błędowi. Propozycja: dodać `BrakNarzedzia` do gałęzi pominięcia obok `PominietoZrodlo` i `PrzekroczonoLimit`, jako osobny, mały pull request przed etapem dwunastym, bo etap dwunasty domyka raport końcowy i wtedy rozjazd między statusem a odczuciem użytkownika byłby najbardziej dotkliwy. Zmiana dotyczy wspólnego mechanizmu, więc wpłynie na FFmpeg i Tesseract tak samo jak na Audiveris — przy realizacji sprawdzić testy oczekujące dziś statusu „blad” dla brakującego narzędzia w tych trzech miejscach i uaktualnić je świadomie, a nie przypadkiem przy okazji.
