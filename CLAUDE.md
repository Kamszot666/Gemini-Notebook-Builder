...
4. ODRZUCONE: renderowanie podglądu partytury przez MuseScore dla materiałów nutowych bez postaci wizualnej, czyli MIDI, MusicXML i Guitar Pro. Stan pierwotny: sekcja piętnasta wymagała zachowania oryginału wizualnego dla każdego materiału nutowego i wskazywała MuseScore uruchamiany z wiersza poleceń jako narzędzie konwersji dającej PDF. Stan po etapie dziesiątym: żaden PDF nie powstaje, MuseScore jest wykrywany, ale nie wywoływany, a do notatnika trafia wyłącznie plik TXT z opisem partytury. Jest to odwrócenie wcześniejszego zapisu, nie jego doprecyzowanie. Powody w kolejności ważności. Po pierwsze, Gemini Notebook nie czyta zapisu nutowego, więc renderowany PDF zająłby slot źródła i nie wniósłby do bazy wiedzy żadnej treści. Po drugie, użytkownik jest osobą niewidomą, więc obraz partytury nie jest dla niego materiałem czytelnym, a jedyną realnie użyteczną postacią jest opis tekstowy; dostępność jest w tym projekcie przesłanką projektową, nie ograniczeniem tymczasowym. Po trzecie, ścieżka renderowania oznaczałaby twardą zależność od zewnętrznego programu wraz z obsługą jego kodów wyjścia, limitów czasu i błędów, dla artefaktu, którego nikt nie otwiera. Warunek rewizji, główny: gdy Gemini Notebook zacznie odczytywać zapis nutowy z pliku PDF albo z obrazu; da się to sprawdzić jednym wgraniem pliku próbnego, a wtedy renderowany PDF przestaje być pustym slotem i staje się treścią. Warunek rewizji, drugi: gdy MuseScore stanie się i tak potrzebny z innego, już przyjętego powodu, bo koszt krańcowy dołożenia renderowania spada wtedy niemal do zera. Prośba użytkownika o wizualne archiwum partytur również wystarcza i nie wymaga osobnego warunku. Anty-warunek, zapisany wprost: powrót do MuseScore w celu dokładnego policzenia taktów w plikach MIDI przez konwersję MIDI na MusicXML nie jest powodem do rewizji. Format MIDI nie zapisuje podziału na takty w ogóle, a import MIDI w MuseScore kwantyzuje i zgaduje granice taktów tak samo jak nasze przeliczenie, tylko innym algorytmem, więc byłaby to zamiana własnego przybliżenia na cudze wraz z dołożeniem zależności. Dla MusicXML liczba taktów wychodzi dokładnie ze znaczników `measure`, a dla Guitar Pro dokładnie z nagłówków taktów.
