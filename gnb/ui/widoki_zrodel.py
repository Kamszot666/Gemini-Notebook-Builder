"""Widoki źródeł projektu: wykaz z działaniami i brakujące pliki.
...
def _sekcja_brakujacych_plikow(brakujace: Sequence[BrakujacyPlikDoWidoku]) -> str:
    ...
    for brak in brakujace:
        zrodla = "; ".join(
            tekst_z_odnosnikami(pochodzenie) for pochodzenie in brak.pochodzenie_zrodel
        )
        skutek = (
...
def _pozycja_zrodla(sciezka: str, zrodlo: ZrodloDoWidoku, token_csrf: str) -> str:
    ...
    if zrodlo.komunikat:
        szczegoly.append(f"<li>Komunikat: {tekst_z_odnosnikami(zrodlo.komunikat)}</li>")
    ...
    if zrodlo.zweryfikowane_recznie:
        ...
        powody = "".join(
            f"<li>{tekst_z_odnosnikami(powod)}</li>" for powod in zrodlo.powody_do_sprawdzenia
        )
        szczegoly.append(
    ...
    dzialania.append(
        f'<form method="post" action="{escapuj(adres_zrodla)}/usun">\n'
        f"{_pole_csrf(token_csrf)}\n"
        f'<button type="submit" aria-describedby="{id_opisu}">'
        "Usuń źródło z projektu</button>\n</form>"
    )

    return (
        f'<li id="zrodlo-{id_el}">\n'
        f'<h3 id="{id_opisu}">{tekst_z_odnosnikami(zrodlo.pochodzenie)}</h3>\n'
        f"<ul>\n{''.join(szczegoly)}\n</ul>\n"
