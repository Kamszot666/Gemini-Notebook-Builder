...
21. `melodia.mid` — plik MIDI typu zerowego z ośmioma nutami, metrum cztery czwarte, tempo sto dwadzieścia, nazwa utworu zapisana w metadanych. Nie zawiera komunikatu tonacji ani zmiany instrumentu. Sprawdza odczyt MIDI oraz zbudowanie opisu tekstowego z metrum, tempem i przybliżoną liczbą taktów wyliczoną z długości nagrania w czasie. Aplikacja nie konwertuje tego pliku żadnym zewnętrznym programem.
22. `melodia.musicxml` — ta sama melodia zapisana jako MusicXML w formacie partwise, z dwoma taktami, tonacją bez oznaczenia trybu, metrum, kluczem i nazwą partii „Fortepian”. Nie zawiera elementu `sound tempo` ani `metronome`. Sprawdza odczyt notacji, dokładną liczbę taktów ze znaczników `measure`, ścieżkę „brak oznaczenia trybu, przyjęto dur” oraz to, że przy braku tempa opis pomija wiersz o tempie, a nie podstawia wartości domyślnej.
23. `nuty_skan.png` oraz `nuty_skan.pdf` — zapis nutowy jako obraz: dwa systemy po pięć linii, główki nut z laskami, tytuł i opis tonacji. W części B etapu dziesiątego pliki te z flagą `--nuty` są naprawdę rozpoznawane programem Audiveris. Uwaga: to jest zapis uproszczony, narysowany programowo, więc służy do sprawdzenia ścieżki przetwarzania, a nie do oceny jakości rozpoznania — patrz sekcja „Czego tu nie ma” poniżej.
24. `nuty_skan_2str.pdf` — ten sam obraz `nuty_skan.png` wstawiony dwukrotnie jako dwie strony jednego pliku PDF. Sprawdza, czy Audiveris przetwarza wielostronicowy plik jednym wywołaniem i łączy strony w jedną ciągłą partyturę, a nie w osobne, równoległe partytury — zweryfikowane ręcznie przy projektowaniu części B: wynikowy plik MusicXML ma cztery takty (dwa z każdej strony) w jednej partii, nie dwie osobne partytury. Wygenerowany od zera skryptem jednorazowym, więc można go bezpiecznie trzymać w publicznym repozytorium.
25. `tabulatura_akordy.gp3`, `tabulatura_akordy.gp4`, `tabulatura_akordy.gp5` — ten sam materiał w trzech wersjach formatu Guitar Pro. Każdy zawiera jedną ścieżkę, osiem taktów, tempo sto dwadzieścia. Sprawdzają odczyt przez bibliotekę PyGuitarPro oraz to, czy program poprawnie rozpoznaje wersję formatu.
26. `tabulatura_efekty.gp5` — czternaście taktów z efektami wykonawczymi. Sprawdza, czy odczyt nie wywraca się na mniej typowych elementach zapisu.
...
Plik `tabulatura_nowy_format.gp` jest atrapą, nie prawdziwym plikiem Guitar Pro w formacie gp albo gpx. Formaty te, czyli wersje szósta i siódma programu, nie są obsługiwane przez PyGuitarPro, a prawdziwego pliku nie da się wygenerować bez samego programu Guitar Pro. Odrzucenie tych formatów następuje po samym rozszerzeniu w warstwie przyjmowania wejścia, zanim cokolwiek próbuje odczytać zawartość, więc kilkubajtowa atrapa o właściwej nazwie w zupełności wystarcza do sprawdzenia tej ścieżki — sprawdzana jest wyłącznie kontrola rozszerzenia, nigdy zawartość pliku.
